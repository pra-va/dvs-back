...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.cors().and().authorizeRequests().antMatchers("/", "/swagger-ui.html", "/createuser").permitAll().and()
				.authorizeRequests().antMatchers("/api/admin").hasRole("ADMIN").antMatchers("/api/user").hasRole("USER")
				...
				}).failureHandler(new SimpleUrlAuthenticationFailureHandler()).loginPage("/api/loginadmin").permitAll()
				.and().logout().logoutUrl("/api/logoutadmin")
